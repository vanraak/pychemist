...
def load(name: str) -> pd.DataFrame:
    if name in datasets:
        with importlib.resources.files("pychemist.data").joinpath(
            f"{name}.parquet"
        ).open("rb") as f:
...
def time_shift(dataframe, variables, id, time, shift=1):
    ...
    if shift == 0:
        raise Exception(
            "Shift value cannot be equal to 0, as it would not change the data."
        )
    ...
    if conflict_columns:
        raise ValueError(
            f"The following lag/lead columns already exist: {', '.join(conflict_columns)}"
        )
...
def summary_no_fe(model):
    ...
    filtered_summary = "\n".join(
        [line for line in summary_str.split("\n") if not line.startswith("C(")]
    )
    ...
    class Pd_Pychemist:
        ...
        def lag(self, variables, identifier, time, shift=1, *, replace=False):
            ...
            if not isinstance(variables, list):
                raise TypeError(
                    "You need to enter a single variable or a list of variables for which lagged variables need to be computed."
                )

            if not isinstance(replace, bool):
                raise TypeError(
                    "The 'replace' argument must be a boolean (True or False)."
                )

            df = (
                self._obj.copy()
            )  # Prevent the original dataframe from getting modified (shouldn't happen, but as a precaution)
            ...
            for var in variables:
                if var not in df:
                    raise KeyError(
                        f"The variable `{var}` does not exist in the DataFrame."
                    )
            ...
            return pd.merge(
                ...
                df_lag,
                how="left",
                left_on=[identifier, time],
                right_on=[identifier, time],
                suffixes=["", suffix],
            )
        ...
        def lead(self, variables, identifier, time, shift=1, *, replace=False):
            ...
            df_lag[time] = (
                df_lag[time] - shift
            )  # Minus shift to generate lead variables
